				     ...
				     .toProvider(new Provider<File>() {
					     public File get() {
						     ...
						     if (mConfig.getGlobalConfig().containsKey(ConfigKeys.ANNOTATION_INDEX)) {
                                 fileName = mConfig.get(ConfigKeys.ANNOTATION_INDEX); 
						     }
						     else {
							     fileName = "/empire.annotation.index";
						     }
                             String filePath = DefaultEmpireModule.class.getResource(fileName).getPath();
